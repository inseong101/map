...
function TrendChart({ rounds, school, sid }) {
  ...
  const prepareAndDrawHistograms = async () => {
    ...
    for (const round of rounds) {
      ...
      // 전국과 학교 각각의 실제 분포 생성
      const natScores = Array.isArray(scoreDistribution.national) ? scoreDistribution.national : [];
      const schScores = (scoreDistribution.bySchool && scoreDistribution.bySchool[schoolCode])
        ? scoreDistribution.bySchool[schoolCode]
        : [];

      const nationalBins = createRealBins(natScores, studentScore);
      const schoolBins   = createRealBins(schScores, studentScore);
  ...
  const createRealBins = (_scores, studentScore) => {
  ...
  const handleCanvasClick = (event, chartType) => {
    ...
    const bins = chartType === 'national'
      ? histogramData[selectedRound].nationalBins
      : histogramData[selectedRound].schoolBins;
    const binWidth = chartW / bins.length;
    ...
    <div>
      ...
      {histogramData[selectedRound] && (
        <div style={{
          ...
          본인 점수: <span style={{ color: '#ef4444', fontWeight: 'bold' }}>
            {histogramData[selectedRound].studentScore}점
          </span>
        </div>
      )}
      ...
      <div style={{
        ...
        <div style={{
          ...
          <canvas
            ...
            style={{ width: '100%', height: '350px', cursor: 'pointer', display: 'block' }}
          />
        </div>
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
          <div style={{ width: '12px', height: '12px', background: '#7ea2ff80', borderRadius: '2px' }}></div>
