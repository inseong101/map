...
function TrendChart({ rounds = [], school = '', sid = '' }) {
  ...
  useEffect(() => {
    (async () => {
      ...
      for (const round of rounds) {
        ...
        // 상위 백분위(전국 기준)
        const percentile = (studentScore != null)
          ? calculatePercentileStrict(natScores, studentScore)
          : null;
  ...
  function drawCurrent(data) {
    ...
    const title = isSchoolMode
      ? `${school} 분포 (유효 ${cur.totalSchool}명)`
    ...
    ctx.fillText(`평균: ${fmt(avg)}점`, W / 2, 40);
    ...
    <div>
      ...
      {current && (
        <div style={{
          ...
          {Number.isFinite(current.studentScore) && current.percentile != null && (
            <>
              {' '}(<span style={{ color: '#7ea2ff', fontWeight: 700 }}>
                전국 상위 {current.percentile.toFixed(1)}%
              </span>)
            </>
          )}
        </div>
